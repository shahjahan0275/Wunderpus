# Version 3
# statistical energy matching for AIDE
# For Ablation Study
'''
import argparse
import math
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------
    # NEW: UPDATE DESTINATION DIRECTORY CREATION
    # ---------------------------------------------------------
    ...
    DEST_DIR_PHASE1 = os.path.join(DESTROOT_DIR, "phase1_semantic") # <--- Added Phase 1 Directory
    os.makedirs(DEST_DIR_EVADED, exist_ok=True)  
    os.makedirs(DEST_DIR_PHASE1, exist_ok=True) # <--- Make Phase 1 Directory
    ...
    for xind, imgfile in enumerate(imagelist):
        ...
        try:
            ...
            # ---------------------------------------------------------
            # NEW: SAVE INTERMEDIATE PHASE 1 IMAGE
            # ---------------------------------------------------------
            base_name = os.path.splitext(imgfile)[0]
            img_phase1_ar = make_image(base_detached)
            ...
            print(f"    [Saved Phase 1 Intermediate: {phase1_img_name}]")
            # ---------------------------------------------------------
            ...
            continue
'''
# To save Phase 1 images for semantic shift also for NDSS submission 20260813
'''
import argparse
import math
import os
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------
    DEST_DIR_PHASE1 = os.path.join(DESTROOT_DIR, "phase1_semantic")
    os.makedirs(DEST_DIR_EVADED, exist_ok=True)  
    ...
    for xind, imgfile in enumerate(imagelist):
        ...
        try:
            ...
            continue
'''
#There are two major issues hidden in your current LPIPS configuration
#that are degrading your image quality:
'''
1. The Spatial Mismatch Bug: Your transform_vae squashes the image to (512, 512)
using Resize((512, 512)). However, your transform_lpips resizes to 512 on the 
shortest edge and then applies a CenterCrop(512). This means LPIPS is forcing 
the VAE to match a cropped version of the image against a squashed version. 
This spatial misalignment causes the optimizer to blur and distort the face 
to compensate.

2. VGG vs. AlexNet: While vgg is standard, the alex (AlexNet) backbone is 
widely considered the gold standard for facial inversion and latent 
optimization (it is what e4e and pSp natively use). vgg tends to 
smooth out high-frequency facial textures, while alex preserves sharp 
details, pores, and natural lighting.

The Tweaks:

To fix this, we will align the LPIPS transforms exactly with the VAE transforms
 and switch the perceptual backbone to AlexNet.
'''
# Version 3_LPIPS tweaks
import argparse
import math
import os
import sys
...
# --- NEW: Replaced VGG with AlexNet for sharper feature matching ---
percept = lpips.PerceptualLoss(model="net-lin", net="alex", use_gpu=True).to(DEVICE)
...
if __name__ == "__main__":
    ...
    imagelist = [f for f in os.listdir(INPUTIMGPATH) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    SEMANTIC_ITERATIONS = 100 
    MIGCOW_ITERATIONS = 150 
    ...
    # --- NEW: Replaced CenterCrop with exact 512x512 Resizing ---
    transform_lpips = transforms.Compose([
