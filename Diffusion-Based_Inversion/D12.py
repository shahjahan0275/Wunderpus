...
try:
    ...
    vae.eval()
    vae.requires_grad_(False) # We will only optimize the latent variable, not the VAE weights
...
if __name__ == "__main__":
    ...
    for xind, imgfile in enumerate(imagelist):
        ...
        try:
            ...
            with torch.no_grad():
                # Direct Latent Inversion in SD VAE
            ...
            with torch.no_grad():
                img_gen_base_raw = vae.decode(latent_in / vae.config.scaling_factor).sample
                # Resize VAE output back to 1024x1024 to match MIG-COW Phase II expected dims
