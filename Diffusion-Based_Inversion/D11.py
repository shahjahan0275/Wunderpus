...
# ---- MAIN SETUP & SCRIPT ----
# ==================================================================================
PROJECT_ROOT = "/forcolab/home/ashahj/adversarialattack"
...
try:
    ...
    classifier.load_state_dict(torch.load('/forcolab/home/ashahj/adversarialattack/stylegan2-pytorch/checkpoint/surrogate_CLIPResNet.pth'))
...
def compute_masked_stats(patches, mask):
    eps = 1e-6
    X = patches * mask
    # Scale cnt by the number of channels (which is patches.shape[1])
    cnt = mask.sum() * patches.shape[1] + eps 
    
    # Sum over Channels (dim=1), Height (dim=2), and Width (dim=3)
...
def extract_radial_dct_stats(tensor_in, patch_size=32):
    """
    [UPDATED] Computes DCT and 4th-order stats directly on all RGB channels separately.
    """
    b, c, h, w = tensor_in.shape # c = 3 (RGB)
    hc, wc = (h // patch_size) * patch_size, (w // patch_size) * patch_size
    
    # Extract patches across all 3 color channels
    ...
    # Calculate DCT for R, G, and B independently
    ...
    # Output remains perfectly formatted as (B, 12) maintaining downstream indexing
    feat = torch.cat([low_s, mid_s, high_s], dim=-1)
    return feat.view(b, -1)

def extract_global_radial_dct_stats(tensor_in):
    """
    [UPDATED] Computes global DCT and 4th-order stats directly on all RGB channels separately.
    """
    b, c, h, w = tensor_in.shape # c = 3 (RGB)
    ...
    # Output remains perfectly formatted as (B, 12)
    feat = torch.cat([low_s, mid_s, high_s], dim=-1)
    return feat.view(b, -1)
