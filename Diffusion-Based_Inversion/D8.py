...
if __name__ == "__main__":
    ...
    for xind, imgfile in enumerate(imagelist):
        ...
        try:
            ...
            # --- AIDE SPECIFIC TARGET: SRM RESIDUAL ALIGNMENT ---
            def loss_fn_srm(d_in):
                """Forces adversarial HPF residuals to match authentic real HPF residuals"""
                img_final_1024 = to_01(base_detached + d_in)
                adv_srm = surrogate_hpf(img_final_1024)
                return 50.0 * F.mse_loss(adv_srm, target_real_srm)
            ...
            for tempi in range(MIGCOW_ITERATIONS):
                ...
                if tempi % 20 == 0:  
                    with torch.no_grad():
                        ...
                        # Monitor new AIDE objectives
                        srm_loss_val = F.mse_loss(surrogate_hpf(img_current), target_real_srm).item()
                        
                        print(f"  [Iter {tempi}] TV: {current_tv:.2f} | Kurt MSE: {current_kurtosis_mse:.4f} | SRM Loss: {srm_loss_val:.4f} | Surrogate 'Fake' Confidence: {prob_fake:.2f}%")
